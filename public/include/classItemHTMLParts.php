...
class ItemHTMLParts
{
     ...
     */
//    public static function getSummaryHTML(DBConnector $db, Site $site, Lang $lngObj)
//    {
//        $lf = "\n";
//
...
//        $summary = viciCommon::htmlentitiesVici($site->getSummary());
//        $siteKinds = new SiteKinds($lngObj);
//
//        $htmlStr = '';
//        //$htmlStr.= '<h2 style="margin:0">'.$site->getTitle().'</h2>'.$lf;
//        $htmlStr .= '<h2 style="margin:0">' . $lngObj->str('Summary') . '</h2>' . $lf;
//        $htmlStr .= '<div class="nearRow">' . $lf;
//        $htmlStr .= '<div class="nearMarkerBox">' . $lf;
//        $htmlStr .= '<img class="' . $markerclass . '" src="data:image/gif;base64,R0lGODlhAQABAIAAAP//////zCH5BAEHAAAALAAAAAABAAEAAAICRAEAOw=="/>' . $lf;
//        $htmlStr .= '</div>' . $lf;
//        $htmlStr .= '<div class="nearTextBoxFull">' . $lf;
//        if ($image->count() == 0) {
//            $htmlStr .= '<p style="width:220px">' . $summary . '</p>' . $lf;
//        } else {
//            $htmlStr .= '<div style="position:relative;width:220px;height:124px;background-color:#5f9cc5">' . $lf;
//            $htmlStr .= '<img src="//static.vici.org/cache/220x124-2' . $image->current()->getPath() . '">';
//            $htmlStr .= '<p style="position:absolute;left:0;right:0;bottom:0;padding:4px;color:black;background-image:url(/images/white_75.png)">' . $summary . '</p>' . $lf;
//            $htmlStr .= '</div>' . $lf;
//        }
//        $htmlStr .= '</div>' . $lf;
//        $htmlStr .= '</div>' . $lf;
//
//        //$htmlStr .= '<div style="float:right;clear:left"><h3>'.$lngObj->str('Data').':</h3><ul class="simpleList"><li><a href="/vici/'.$site->getId().'/rdf">RDF/XML</a></li><li><a href="/vici/'.$site->getId().'/kml">KML</a></li></ul></div>'.$lf;
//
//        $htmlStr .= '<div>' . $lf;
//        $htmlStr .= '<h3 style="margin-bottom:2px;clear:left">' . $lngObj->str('Class') . ':</h3>' . $lf;
//        $htmlStr .= '<ul class="simpleList">' . $lf;
...
//        }
//        $htmlStr .= '<li>' . $lngObj->str('accuracy_' . $site->getLocationAccuracy()) . '</li>' . $lf;
//
//        $htmlStr .= '</ul>' . $lf;
//
//        $htmlStr .= '<div style="float:right">(' . $lngObj->str('see also') . ' <a href="http://pelagios.org/peripleo/map#places=http:%2F%2Fvici.org%2Fvici%2F' . $site->getId() . '">PELAGIOS</a>)</div>';
//        $htmlStr .= '<h3 style="margin-bottom:2px">' . $lngObj->str('Identifiers') . ':</h3>' . $lf;
//
//        $htmlStr .= '<div id="ids" style="height:5em;overflow:auto">';
//        $htmlStr .= '<ul class="simpleList">' . $lf;
//        $htmlStr .= '<li><a href="//vici.org/vici/' . $site->getId() . '">vici:id=' . $site->getId() . '</a></li>' . $lf;
//
//        if ($site->getExternalIdsObj()->getPleiades()) {
//            $htmlStr .= '<li><a href="' . $site->getExternalIdsObj()->getPleiadesUrl() . '">' . $site->getExternalIdsObj()->getPleiadesTag() . '</a></li>' . $lf;
//        }
//        if ($site->getExternalIdsObj()->getWikidata()) {
//            $htmlStr .= '<li><a href="' . $site->getExternalIdsObj()->getWikidataUrl() . '">' . $site->getExternalIdsObj()->getWikidataTag() . '</a></li>' . $lf;
//        }
//        if ($site->getExternalIdsObj()->getLivius()) {
//            $htmlStr .= '<li><a href="' . $site->getExternalIdsObj()->getLiviusUrl() . '">' . $site->getExternalIdsObj()->getLiviusTag() . '</a></li>' . $lf;
//        }
//        if ($site->getExternalIdsObj()->getDare()) {
//            $htmlStr .= '<li><a href="' . $site->getExternalIdsObj()->getDareUrl() . '">' . $site->getExternalIdsObj()->getDareTag() . '</a></li>' . $lf;
//        }
//        if ($site->getExternalIdsObj()->getRomaq()) {
//            $htmlStr .= '<li><a href="' . $site->getExternalIdsObj()->getRomaqUrl() . '">' . $site->getExternalIdsObj()->getRomaqTag() . '</a></li>' . $lf;
//        }
//
//
//        $htmlStr .= '</ul>' . $lf;
