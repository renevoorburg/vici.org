...
function normalizedArea() {       
    this.fill = function (bounds) {
        ...
        this.yMin = Math.floor(latSW/dLat);
        this.xMin = Math.floor(lngSW/dLng);
        this.yMax = Math.ceil(latNE/dLat);
        this.xMax = Math.ceil(lngNE/dLng);
        this.latSW = this.yMin * dLat;
        this.lngSW = this.xMin * dLng;
        this.latNE = this.yMax * dLat;
        this.lngNE = this.xMax * dLng;
    }
    ...
    this.equals = function (o) {
        var result = (
            o &&
            (this.yMin && o.yMin && (this.yMin == o.yMin)) &&
            (this.xMin && o.xMin && (this.xMin == o.xMin)) &&
            (this.yMax && o.yMax && (this.yMax == o.yMax)) &&
            (this.xMax && o.xMax && (this.xMax == o.xMax))
        );
        return result;
    }
} //normalizedArea
...
var vici = {
    ...
    markerArr:      [],
    polylineArr:    [],
    areaLoaded:     new normalizedArea(),
    areaRequested:  new normalizedArea(),
    popupsArr:      [],
...
function showPage(blurb) {
        ...
        if (d.properties.images[0]) {
            ...
            for (var i in d.properties.images) {
                ...
                if (img.isdefault != '1') {   // default image is already shown, so skip adding it
                    ...
                    content += '<img id="img'+i+'" class="popable" src="//static.vici.org/cache/260x0-3'+img.path+'">';
                }
    ...
    if(blurb) {
        ...
        setInnerHTML('text', '<p>Vici.org mobile</p><p>V 2.0 beta 4 - René Voorburg 2013 - rene@digitopia.nl</p><p>Visit http://vici.org/</p>');
        ...
        if (vici.markerArr[vici.selectedMarkerId].picture) {
            setInnerHTML('defaultimage', '<img id="img0" class="popable" src="//static.vici.org/cache/260x146-2'+vici.markerArr[vici.selectedMarkerId].picture+'" width="260" height="146">');
        } else {
...
function loadData(map) {
    ...
    function markerClicked(marker, center) {
        ...
        if (marker.id ==  vici.selectedMarkerId ) {
            ...
            if (marker.picture) {
                ...
                content += '<img  src="//static.vici.org/cache/260x146-2'+marker.picture+'" width="260" height="146">';
    ...
    // remove unused markers from map, save memory:
    function dropSuperfluousMarkers() {
        var bounds = new google.maps.LatLngBounds(
            new google.maps.LatLng(vici.areaLoaded.latSW, vici.areaLoaded.lngSW), 
            new google.maps.LatLng(vici.areaLoaded.latNE, vici.areaLoaded.lngNE)
        );
        var zoomlevel = map.getZoom();
        for (var i in vici.markerArr) {
            if (!bounds.contains(vici.markerArr[i].getPosition()) || (vici.markerArr[i].zoom_small > zoomlevel)) {
                // remove marker
                google.maps.event.clearInstanceListeners(vici.markerArr[i]);
                vici.markerArr[i].setMap(null);
                delete vici.markerArr[i];
            }
        }
    ...
    function setFeatures(d) {
        ...
        createMarkers(d);
        createLines(d);
        vici.areaLoaded.assign(vici.areaRequested);
    ...
    function createMarkers (d) {
        ...
        function plotMarkers(feature) {
            if (!vici.markerArr[feature.properties.id]) {
                ...
                var marker = new google.maps.Marker({
                    ...
                    image_small: markerImages[feature.properties.kind].small,
                    image_normal: markerImages[feature.properties.kind].normal,
                    state_high: false,
                    icon: null,
        ...
        processItems(d.features, plotMarkers, 100); 
    } // end createMarkers  
    
    function createLines(d) {
        
        var calc = function(zoomlevel) {
            var exponent =  (1 + zoomlevel - this.growZoom);
            if (exponent < 0) { exponent = 0; };
            return this.nominalWeight * Math.pow(2, exponent);
        }
        ...
        function plotLine(line) {
            ...
            if (!vici.polylineArr[line.id] || (vici.polylineArr[line.id] && vici.polylineArr[line.id].expire <= zoomlevel)) {
                ...
                switch (line.kind) {
                    case 'aqueduct':
                        polylineOptions.strokeColor = "#00ECF7";
                        polylineOptions.nominalWeight = 2;
                        break;
                    case 'canal':
                        polylineOptions.strokeColor = "#1111FF";
                        polylineOptions.nominalWeight = 3;
                        polylineOptions.growZoom = 16;
                        break;
                    case 'road':
                        break;
                    case 'wall':
                        polylineOptions.growZoom = 17;
                        polylineOptions.strokeColor = "#C4A548";
                        break;
                    case 'other':
                        polylineOptions.nominalWeight = 2;
                        polylineOptions.growZoom = 19;
                        polylineOptions.strokeColor = "#fdff0c";
                        break;
                };
                polylineOptions.strokeWeight = polylineOptions.calcWeight(zoomlevel);
    ...
    vici.areaRequested.fill(map.getBounds());
    if (!vici.areaRequested.equals(vici.areaLoaded)) {  
        ajax({
            url: base+"/data.php?numeric&bounds="+vici.areaRequested.latSW+","+vici.areaRequested.lngSW+","+vici.areaRequested.latNE+","+vici.areaRequested.lngNE+"&zoom="+map.getZoom()+"&lang="+vici.lang,
            dataType: 'json',
            success: setFeatures
        });
    };
...
function initialize() {
    ...
    google.maps.event.addListener(vici.map, 'zoom_changed', (function() { 
        ...
        for (var i in vici.polylineArr) {
            vici.polylineArr[i].strokeWeight = vici.polylineArr[i].calcWeight(zoomlevel);
