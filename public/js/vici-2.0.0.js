...
function ViciWidget(element, options) {
    ...
    var baseUrl = window.location.hostname.match(/\.local$/) == '.local' ? window.location.protocol + '//vici.local' : window.location.protocol + '//vici.org';

    var lang =  (options.lang) ? options.lang : (navigator.language || navigator.userLanguage).substring(0, 2) ;
    ...
    var session = (sessionStorage.getItem('session'))  ? JSON.parse(sessionStorage.getItem('session')) : {
    ...
    var mapState = {
        ...
        numHighlights : (function(){
            let available = Math.floor((document.getElementById(element).clientHeight - 224) / 175);
    ...
    var vectorLayerMarkers = new ol.layer.Vector({
    ...
    function selectIcon(marker, selected) {
        ...
        var visibilityMatch = (session.filter.visibility == "anyVisibility") || (marker.vici.visible);
        var eraMatch =  (session.filter.era == "anyEra") ||
    ...
    function updateDataLayer() {
        ...
        function setFeatures(d) {
            ...
            if (!mapState.initialized && session.selectedMarkerId) {
            ...
            for (let i in d.lines) {
                ...
                if (!vectorSourceLines.getFeatureById(feature.id)) {
                    ...
                    var line = vectorSourceLines.getFeatureById(feature.id);
                    var lineCoordinates=[];
        ...
        $.ajax({
            url: baseUrl + "/data.php?numeric"+mapState.model+"&perspective="+mapState.perspective+"&bounds=" + SW[1] + "," + SW[0] + "," + NE[1] + "," + NE[0] + "&zoom=" + zoomlevel + mapState.langReq,
            dataType: 'json',
            success: setFeatures
        });
    }
    ...
    map.on('click', function(evt) {
        ...
        if (feature) {
            if (feature.vici.markerId) {
                // a line was clicked:
                var marker = vectorSourceMarkers.getFeatureById(feature.vici.markerId);
    ...
    function getHighlights() {
        ...
        if (mapState.numHighlights > 0) {
            ...
            var SW = ol.proj.toLonLat([extent[0], extent[1]]);
            var NE = ol.proj.toLonLat([extent[2], extent[3]]);
            var bounds = SW[1] + "," + SW[0] + "," + NE[1] + "," + NE[0];

            var requrl = baseUrl + "/highlight.php?numeric&perspective=" + mapState.perspective + "&bounds=" + bounds + "&zoom=" + zoomlevel + "&n=" + mapState.numHighlights + "&era=" + session.filter.era + "&visibility=" + session.filter.visibility + mapState.langReq;
            if (session.selectedMarkerId && vectorSourceMarkers.getFeatureById(session.selectedMarkerId)) {
    ...
    var highlightsbox = document.createElement('div');
    ...
    function updateInfobox(feature) {
        ...
        if (session.selectedMarkerId && vectorSourceMarkers.getFeatureById(session.selectedMarkerId)) {
            ...
            var previouslySelectedMarker = vectorSourceMarkers.getFeatureById(session.selectedMarkerId);
