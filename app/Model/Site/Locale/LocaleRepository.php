...
class LocaleRepository
{
    ...
    {
        $stmt = $this->db->prepare("SELECT t.ptxt_lang AS language, s.psum_pnt_name AS title, s.psum_short AS summary, t.ptxt_full AS description FROM points p
        LEFT JOIN psummaries s ON s.psum_pnt_id = p.pnt_id
        LEFT JOIN ptexts t ON t.ptxt_pnt_id = s.psum_pnt_id AND t.ptxt_lang = s.psum_lang
        WHERE p.pnt_id = :id");
