...
/**
Promotes or unpromotes a point (toggle). JSON output.
 *
 * TODO: this is basically the same thing as togglehide
 */
...
try {
    ...
    $sql = "SELECT pnt_promote FROM points WHERE pnt_id='".$id."'";
    $result = $db->query($sql);
    list($pnt_promote) = $result->fetch_row();
    ...
    $sql = "UPDATE points SET pnt_promote='$pnt_promote_new' WHERE pnt_id='$id'";
    $result = $db->query($sql);

    echo "{ \"promote\": \"$pnt_promote_new\" }";
